...
<!--
        My Additional Menu - Select Attribute to Show
-->
        <div class="form-group">
            <label class="control-label"><?= t('Attribute to Display') ?></label>
            <?php
                echo $form->select('attributeToAnalyse', $attributesKeysToAnalyse, null);
            ?>
        </div>

        <div>
            <?php 
                echo $attributeToAnalyse;
            ?>
        </div>
<!--
-->
                        ...
                        foreach ($pages as $pc) {
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div> Attribute: <?= $attributeToAnalyse ?></div>
                                    <div> Value(s): <?= $pc->getAttribute($attributeToAnalyse, 'display')?></div>
                                </td>
                                <!-- -->
                                <?php
                                foreach ($targetList as $sc) {
                                        ...
                                        <td>
                                            <div data-multilingual-page-section="<?= $sc->getCollectionID() ?>" data-multilingual-page-source="<?= $pc->getCollectionID() ?>">
                                                <div data-wrapper="page">
                                                    ...
                                                    if ($cID) {
                                                        $p = \Page::getByID($cID);
                                                        echo '<a href="' . $nav->getLinkToCollection($p) . '">' . $p->getCollectionName() .$p->getAttribute('ts_view_count_np', 'display'). '</a>';
                                                        //echo ;
                                                        ?>
                                                            <div> Attribute: <?= $attributeToAnalyse ?></div>
                                                            <div> Value(s): <?= $p->getAttribute($attributeToAnalyse, 'display')?></div>
                                                        <?php
                                                    } elseif ($cID === '0') {
                                                        echo t('Ignored');
                                                    }
